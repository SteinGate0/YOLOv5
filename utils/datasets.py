...
def exif_size(img):
    ...
    try:
        rotation = dict(img._getexif().items())[orientation] # 调整数码相机照片方向， 因为框是水平的，如果斜着拍的数码照片容易识别不出来
...
cap.read(frame) 结合grab和retrieve的功能，抓取下一帧并解码，结合前两个函数，既做帧获取，又解码
"""
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        self.rect = np.unique(s, axis=0).shape[0] == 1  # rect inference if all shapes equal  检查读取的帧的形状是否一样
        if not self.rect:
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.hyp = hyp # 超参数
        self.image_weights = image_weights # 图片采样权重 采样的时候，有的类别可以采的多一点，有的类别采的少一点
        self.rect = False if image_weights else rect # 矩形训练 如果是False那么不采取ar排序的矩形训练了
        # mosaic数据增强
        ...
        cache_path = str(Path(self.label_files[0]).parent) + '.cache'  # cached labels  标签文件的缓存文件，加载方便
        if os.path.isfile(cache_path):
            cache = torch.load(cache_path)  # load
            if cache['hash'] != get_hash(self.label_files + self.img_files):  # dataset changed
                cache = self.cache_labels(cache_path)  # re-cache   数据集变了hash值也会变，新计算cache
        else:
        ...
        for i, file in pbar:
            ...
            if l is not None and l.shape[0]:
                ...
                if np.unique(l, axis=0).shape[0] < l.shape[0]:  # duplicate rows 重复的行，标注框标注了两次
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                ...
                img = (img * r + img2 * (1 - r)).astype(np.uint8) # mixup 两幅图片加权生成新的图片
        ...
        # 初始化标签框对应的图片序号，(变成六列)配合下面的collate_fn使用
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        # 根据ratio选择不同的插值方式
        if r != 1:  # always resize down, only resize up if training with augmentation  一般是做向下插值，但是做数据增强的时候向上插值
...
def load_mosaic(self, index):
    ...
    s = self.img_size
    # 随机取mosaic中心点
    yc, xc = [int(random.uniform(-x, 2 * s + x)) for x in self.mosaic_border]  # mosaic center x, y 2*s宽高扩大一倍再随机取点
    # 随机取其它三张图片的索引
    ...
    for i, index in enumerate(indices):
        ...
        if x.size > 0:  # Normalized xywh to pixel xyxy format
            labels[:, 1] = w * (x[:, 1] - x[:, 3] / 2) + padw # 推算正确
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    ...
    elif scaleFill:  # stretch  是否任由图片进行拉伸
        dw, dh = 0.0, 0.0
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            # 仿射变换函数，可实现旋转，平移，缩放；变换后的平行线依旧平行  M矩阵最后两列与中心点，平移有关，切片后只与scale有关
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=20, area_thr=0.1):  # box1(4,n), box2(4,n)  box1是数据增强前的目标框，box2是数据增强后的目标框
    # Compute candidate boxes: box1 before augment, box2 after augment, wh_thr (pixels), aspect_ratio_thr, area_ratio
    w1, h1 = box1[2] - box1[0], box1[3] - box1[1]
    w2, h2 = box2[2] - box2[0], box2[3] - box2[1]
    ar = np.maximum(w2 / (h2 + 1e-16), h2 / (w2 + 1e-16))  # aspect ratio
    return (w2 > wh_thr) & (h2 > wh_thr) & (w2 * h2 / (w1 * h1 + 1e-16) > area_thr) & (ar < ar_thr)  # candidates  wh_thr像素大于2

# cutout数据增强 把图片上的一部分去除，去除的部分填充随机的颜色
def cutout(image, labels):
    ...
    for s in scales:
        ...
        image[ymin:ymax, xmin:xmax] = [random.randint(64, 191) for _ in range(3)]  # 切掉的部分用随机的颜色填充

        # return unobscured labels
        if len(labels) and s > 0.03:
            box = np.array([xmin, ymin, xmax, ymax], dtype=np.float32)
            ioa = bbox_ioa(box, labels[:, 1:5])  # intersection over area
            labels = labels[ioa < 0.60]  # remove >60% obscured labels   如果切的面积太大，就不考虑这种目标框，对训练有影响
