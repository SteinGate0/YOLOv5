...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # p是原图片路径（含文件名）
            x = Path(p).name
            save_path = str(Path(out) / Path(p).name)      # 9_output/img/文件名

            # 设置保存框坐标txt文件的路径
            txt_path = str(Path(out).parent.joinpath('txt')/ Path(p).stem) + ('_%g' % dataset.frame if dataset.mode == 'video' else '')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='D:\PyCharm\MyProjcet\Mask_test\yolov5-src\weights\yolov5l.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='D:\PyCharm\MyProjcet\Mask_test\yolov5-src\inference\images/away.jpg', help='source')  # file/folder, 0 for webcam
    ...
    parser.add_argument('--save-dir', type=str, default='D:\PyCharm\MyProjcet\Mask_test\yolov5-src\inference\output', help='directory to save results')
