...
def train(hyp, opt, device, tb_writer=None):
    ...
    for k, v in model.named_parameters():
        print(k)
    freeze = ['model.%s.' % x for x in range(13,21) ]  # parameter names to freeze (full or partial)
    if any(freeze):
        for k, v in model.named_parameters():
    ...
    hyp['cls'] *= nc / 2.  # scale coco-tuned hyp['cls'] to current dataset
    # 设置类别数，超参数
...
if __name__ == '__main__':
    ...
    image-weights：从上一轮中测试过程中表现不好的图片，下一轮对这些图片加权重训练，默认False，效果不一定得到保证
    """
    ...
    parser.add_argument('--data', type=str, default='data/new_mask_data.yaml', help='data.yaml path')
