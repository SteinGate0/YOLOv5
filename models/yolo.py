...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        # 所以anchors的更新只在正向传播中更新，反向传播就不需要这些值了
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference 推理的时候
                if self.grid[i].shape[2:4] != x[i].shape[2:4]:
                    self.grid[i] = self._make_grid(nx, ny).to(x[i].device) # 调用make_grid函数构造网格
    ...
    def _make_grid(nx=20, ny=20):
        # 划分为单元网格
        yv, xv = torch.meshgrid([torch.arange(ny), torch.arange(nx)]) # torch.meshgrid在图像上划分单元网格
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            s = 128  # 2x min stride
            # m.stride = [8,16,32] X倍的下采样
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward 
            # anchor大小计算, 例如 [10, 13] --> [1.25, 1.625]
            m.anchors /= m.stride.view(-1, 1, 1)   # 使用stride进行归一化后anchor的大小
    ...
    def forward(self, x, augment=False, profile=False):
        if augment: # TTA (Test Time Augmentation)   测试、推理的时候也可以进行数据增强
            img_size = x.shape[-2:]  # height, width
            s = [1, 0.83, 0.67]  # scales
            f = [None, 3, None]  # flips (2-ud, 3-lr)   filps 反转
            y = []  # outputs
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C3]:
            ...
            args = [c1, c2, *args[1:]]  # 上一层的channel , 卷积核个数 , 紧跟的参数
            if m in [BottleneckCSP, C3]:
                args.insert(2, n)  # 在args位置2插入值n
                n = 1
