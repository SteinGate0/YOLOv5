...
# 深度可分离卷积，未使用
def DWConv(c1, c2, k=1, s=1, act=True): # c1,c2分别是输入输出channel的值，k=1是卷积核kenel，s=1是步长stride
...
class Bottleneck(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcutd短接, groups, expansion
...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)   列表[起始:终止:步长] my_list[1::3] # 等同于 my_list[1:-1:3]
...
# 用于第二级分类，车牌识别，目标检测+检测车牌上的字，比较简单可以重新改写
